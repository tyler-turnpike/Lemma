---
name: hono
description: Use when building Hono web applications or when the user asks about Hono APIs, routing, middleware, JSX, validation, testing, or streaming. TRIGGER when code imports from 'hono' or 'hono/*', or user mentions Hono.
---

# Hono Skill

Build Hono web applications. This skill provides inline API knowledge for AI.
...
> **Modified for the Lemma repository** (vendored from honojs/skills@8b1938be37331c68a02c3b75896b2ea3839d7d09).
> The upstream "Hono CLI" section was removed during vetting: it installed the unpinned prerelease
> `@hono/cli@next` into the project, ran `npx hono agent-context`, and told the agent to follow that
> command's output as instructions. See `../SOURCES.md`. To exercise routes without a server, use
> `app.request()` in a Vitest test (see "Testing with app.request()" below).
